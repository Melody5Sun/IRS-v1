...
- 面试题 `role`（单值 TEXT）改为 `roles_json`（JSON 数组，模型字段 `roles: list[str]`）：215 条逐题按题干重新判定，可对应多个岗位（83 条多岗位）；数据结构/算法、C/C++/Java/Python 语言基础等匹配不到具体岗位的题统一标为 `["编程基础题"]`（26 条，常量 `GENERAL_PROGRAMMING_ROLE`），空列表会被校验器自动补成该值。旧库启动时由 `_migrate_interview_role_to_roles_json` 迁移（旧值转成单元素数组后删列）

...
- 入口：`filter_jobs(profile: UserProfile, db_path: str) -> {"job_ids": [...]}`
- 学生画像直接用 `app.schemas.profile.UserProfile`
- 岗位数据：`jobs INNER JOIN job_analysis`（没有分析记录的岗位不输出），数据库以只读方式打开
...
| 6 | 行业 | `industry` 不在学生 `target_industries` 中 | 学生列表为空 |
...
4. 在 `tests/test_rule_engine.py` 里补"应剔除"和"应保留"两类用例。
